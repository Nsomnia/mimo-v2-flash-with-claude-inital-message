...
Result<void> ProjectMBridge::init(const ProjectMConfig& config) {
    ...
    projectm_set_preset_duration(projectM_,
                                 0); // Disable internal timer, we use our own
    ...
    if (config.useDefaultPreset) {
        ...
        // Check if a preset was already selected (e.g., from command-line
        // before scanning) If currentIndex is not 0, a preset was already
        // selected
        bool presetAlreadySelected =
                presets_.currentIndex() > 0 && presets_.current() != nullptr;
        ...
        if (presetAlreadySelected) {
            ...
            if (presets_.selectByName(config.forcePreset)) {
                ...
                LOG_WARN(
                        "Could not find forced preset: {}, using first "
                        "available",
                        config.forcePreset);
...
void ProjectMBridge::render() {
    ...
    LOG_DEBUG("ProjectMBridge::render(): Rendering preset '{}' (handle={})",
              presetName,
              (void*)projectM_);
    projectm_opengl_render_frame(projectM_);
    LOG_DEBUG("ProjectMBridge::render(): Completed frame for preset '{}'",
              presetName);
}
...
void ProjectMBridge::addPCMData(const f32* data, u32 samples, u32 channels) {
    ...
    if (channels == 1) {
        ...
        projectm_pcm_add_float(
                projectM_, data, samples * channels, PROJECTM_STEREO);
    }
}
...
void ProjectMBridge::addPCMDataInterleaved(const f32* data,
    ...
    if (!projectM_) {
        LOG_DEBUG(
                "ProjectMBridge::addPCMDataInterleaved: projectM_ is null, "
                "skipping");
        return;
    }

    LOG_DEBUG("ProjectMBridge::addPCMDataInterleaved: {} frames, {} channels",
              frames,
              channels);
...
void ProjectMBridge::loadPreset(const fs::path& path, bool smooth) {
    ...
    projectm_load_preset_file(
            projectM_, path.c_str(), false); // Force non-smooth
...
void ProjectMBridge::previousPreset(bool smooth) {
    ...
    LOG_DEBUG("ProjectMBridge::previousPreset() called, shuffleEnabled={}",
              shuffleEnabled_);
