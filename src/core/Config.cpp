...
Result<void> Config::loadDefault() {
    ...
    if (fs::exists(systemDefault)) {
        ...
        if (!ec) {
            LOG_INFO("Copied default config to user directory: {}",
                     defaultPath.string());
            return load(defaultPath);
        } else {
    ...
    if (fs::exists(projectDefault)) {
        ...
        if (!ec) {
            LOG_INFO("Copied project default config to user directory: {}",
                     defaultPath.string());
            return load(defaultPath);
        } else {
...

        {
            ...
            if (!file) {
                return Result<void>::err(
                        "Failed to open temp config file for writing");
            }
            file << tbl;
        }
...
void Config::parseRecording(const toml::table& tbl) {
    if (auto rec = tbl["recording"].as_table()) {
        ...
        if (auto video = (*rec)["video"].as_table()) {
            recording_.video.codec =
                    get(*video, "codec", std::string("libx264"));
            recording_.video.crf = std::clamp(get(*video, "crf", 23u), 0u, 51u);
            recording_.video.preset =
                    get(*video, "preset", std::string("ultrafast"));
            recording_.video.pixelFormat =
                    get(*video, "pixel_format", std::string("yuv420p"));

            u32 w = std::clamp(get(*video, "width", 1280u), 320u, 7680u);
            if (w % 2 != 0)
                w++; // FFmpeg H.264 likes even dimensions
...
void Config::parseKeyboard(const toml::table& tbl) {
    if (auto kb = tbl["keyboard"].as_table()) {
        ...
        keyboard_.toggleFullscreen =
                get(*kb, "toggle_fullscreen", std::string("F"));
...
toml::table Config::serialize() const {
    ...
    // General
    root.insert("general", toml::table{{"debug", debug_}});

    // Audio
    root.insert(
            "audio",
            toml::table{{"device", audio_.device},
    ...
    root.insert(
            "visualizer",
            toml::table{{"preset_path", visualizer_.presetPath.string()},
                        {"width", static_cast<i64>(visualizer_.width)},
                        {"height", static_cast<i64>(visualizer_.height)},
                        {"fps", static_cast<i64>(visualizer_.fps)},
                        {"beat_sensitivity",
                         static_cast<double>(visualizer_.beatSensitivity)},
                        {"preset_duration",
                         static_cast<i64>(visualizer_.presetDuration)},
                        {"smooth_preset_duration",
    ...
    root.insert("keyboard",
                toml::table{{"play_pause", keyboard_.playPause},
